...
class Sphere : public Object {
  public:
    ...
    bool intersect(const Ray& r, Interval ray_t, IntersectionRecord& rec) const override {
        ...
        double sin_theta = sqrt(saturate(1.0 - rec.p.y() * rec.p.y()));
        double sin_phi = sin_theta == 0 ? 0 : rec.normal.x() / sin_theta;
        double cos_phi = sin_theta == 0 ? 1 : rec.normal.z() / sin_theta;
        
        rec.dpdu = Vector3(rec.normal.z(), 0, -rec.normal.x()) * radius * tau;
        rec.dpdv = Vector3(rec.normal.y() * sin_phi, -sin_theta, rec.normal.y() * cos_phi) * radius * pi;
